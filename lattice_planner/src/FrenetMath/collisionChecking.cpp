...
//     disk2.CrtPoint_.x = RearAxleMidPoint_.x + Distance_ * cos(RearAxleMidPoint_.yaw);
//     disk2.CrtPoint_.y = RearAxleMidPoint_.y + Distance_ * sin(RearAxleMidPoint_.yaw);
//     disk2.radius = disk1.radius;

//     disk3.CrtPoint_.x = RearAxleMidPoint_.x + 2 * Distance_ * cos(RearAxleMidPoint_.yaw);
//     disk3.CrtPoint_.y = RearAxleMidPoint_.y + 2 * Distance_ * sin(RearAxleMidPoint_.yaw);
...
VehicleDisks disksTrajectory::generateDisks(CartesianPose &RearAxleMidPoint_){
    ...
    VehicleDisks_.disk1.radius_= sqrt(H_*H_ + VehicleWidth_*VehicleWidth_/4.0);
    ...
    VehicleDisks_.disk2.CrtPoint_.x = RearAxleMidPoint_.x + Distance_ * cos(RearAxleMidPoint_.yaw);
    VehicleDisks_.disk2.CrtPoint_.y = RearAxleMidPoint_.y + Distance_ * sin(RearAxleMidPoint_.yaw);
    ...
    VehicleDisks_.disk3.CrtPoint_.x = RearAxleMidPoint_.x + 2 * Distance_ * cos(RearAxleMidPoint_.yaw);
    VehicleDisks_.disk3.CrtPoint_.y = RearAxleMidPoint_.y + 2 * Distance_ * sin(RearAxleMidPoint_.yaw);
    ...
    VehicleDisks_.BigDisk.radius_ = sqrt(VehicleLength_ * VehicleLength_ / 4.0 + VehicleWidth_*VehicleWidth_/4.0);
...
CollisionChecker::CollisionChecker(CartesianPath & ctsPath, 
...
bool CollisionChecker::performChecking(){
    ...
    for (auto m:disksTrajectory_){
        ...
        for (auto q:obstacles_.points){
            ...
            double tmpDistance = pow(BigDisk.CrtPoint_.x - q.x, 2) + pow(BigDisk.CrtPoint_.y - q.y, 2);
            if (tmpDistance > pow(BigDisk.radius_,2)){
                ...
                tmpDistance = pow(disk1.CrtPoint_.x - q.x,2) + pow(disk1.CrtPoint_.y - q.y, 2);
                if (tmpDistance < pow(disk1.radius_,2)) {
                    // ROS_INFO("the min distance is: %f", sqrt(tmpDistance));
                    return false;
                }
                tmpDistance = pow(disk2.CrtPoint_.x - q.x,2) + pow(disk2.CrtPoint_.y - q.y, 2);
                if (tmpDistance < pow(disk2.radius_,2)) {
                    // ROS_INFO("the min distance is: %f", sqrt(tmpDistance));
                    return false;
                }
                tmpDistance = pow(disk3.CrtPoint_.x - q.x,2) + pow(disk3.CrtPoint_.y - q.y, 2);
                if (tmpDistance < pow(disk1.radius_,2)) {
                    // ROS_INFO("the min distance is: %f", sqrt(tmpDistance));
                    return false;
                }

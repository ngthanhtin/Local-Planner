...
//  std::cout << "created Voronoi Diagram in ms: " << d * 1000 << std::endl;
  ...
  if (!Constants::manual && listener.canTransform("/map", ros::Time(0), "/base_link", 
                                                  ros::Time(0), "/map", nullptr)) {
...
void Planner::plan() {
  ...
  if (validStart && validGoal) {
    ...
    smoothedPath.clear();
    // FIND THE PATH
    Node3D* nSolution = Algorithm::hybridAStar(nStart, nGoal, nodes3D, nodes2D, 
